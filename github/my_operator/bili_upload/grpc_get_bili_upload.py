...
class renew:
    ...
    async def get_dynamic_detail_with_proxy(self, dynamic_id, _cookie='', _useragent='', dynamic_type=2):
        ...
        try:
            ...
            if is_liked:
                ...
                is_liked = 0
            if relation != 1:
                logger.info('未关注的response')
                logger.info(f'https://space.bilibili.com/{author_uid}')
        except Exception as e:
    ...
    async def judge_lottery(self, dynamic_id, dynamic_type: int = 2, is_lot_orig=False):
        ...
        suffix = ''
        if dynamic_detail:
            dynamic_id = dynamic_detail['dynamic_id']  # 获取正确的动态id，不然可能会是rid或者aid
            ...
            if dynamic_content != '':
                ...
                logger.info(f'https://t.bilibili.com/{dynamic_id}')
                logger.info('动态内容为空')
                deadline = None
    ...
    async def main(self):
        ...
        await asyncio.gather(*[self.get_user_space_dynamic_id(i) for i in self.uidlist])
