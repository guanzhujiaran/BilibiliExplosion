...
replycontent = ['[原神_欸嘿]', '[原神_喝茶]', '好耶', '来了', '冲鸭', '来了来了', '贴贴', '冲', '[原神_哼]', '[原神_嗯][原神_哇][原神_哼]',
                '[原神_哇][原神_哼]', '[原神_哇]', '万一呢']
...
def getcomment_dynamic(text):  # 直接返回动态内容
    comment_json = json.loads(text)
    itemstr = comment_json.get('data').get('card').get('card')
    item_json = json.loads(itemstr)
    dictresult = item_json.get('item').get('description')
    return dictresult
...
def huifuneirong(content, detail, _type):  # 回复内容的判断
    ...
    if pinglunreq.text != '{"code":-404,"message":"啥都木有","ttl":1}':
        prev = re.findall('prev":(.*?),', pinglunreq.text)
        if prev != []:
            prev = prev[0]
            while True:
                i += 1
                pnlist = list(range(20, int(prev) - 20))
                if pnlist == []:
                ...
                pinglunheader = {
                    'user-agent': fakeuseragent}
                pinglunurl = 'https://api.bilibili.com/x/v2/reply/main?next=' + str(pn) + '&type=' + str(
                    ctype) + '&oid=' + str(oid) + '&mode=2&plat=1&_=' + str(time.time())
                pinglundata = {
                    'jsonp': 'jsonp',
                    'next': pn,
                    'type': ctype,
                ...
                if pinglunreq.text != '{"code":-404,"message":"啥都木有","ttl":1}':
                    pinglunreq1 = ''.join(
                        re.findall(r'{"code":0,"message":"0","ttl":1,"data":(.*),"top":', pinglunreq.text)[0])
                ...
                if pinglun != []:
                    if pinglun != []:
                        for i in range(pinglunzhuanbiancishu):
                            msg = ''.join(random.choice(pinglun))
...
def choujiangxinxipanduan(tcontent):  # 动态内容过滤条件
    matchobj_46 = re.match('.*抽奖.*抽.*小伙伴.*评论.*转发.*', tcontent)
    matchobj_45 = re.match('.*关注.*一键三连.*分享.*送.*', tcontent)
    ...
    if (
            matchobj_46 == None and matchobj_45 == None and matchobj_44 == None and matchobj_43 == None and matchobj_42 == None and matchobj_41 == None and matchobj_40 == None and matchobj_39 == None and matchobj_38 == None and matchobj_37 == None and matchobj_36 == None
...
def comment(dy, msg, type, detail):  # 发送评论
    ...
    time.sleep(random.choice(sleeptime))
    commentrpid = re.findall('"rpid":(.*?),', reqcomment.text)
    if commentrpid == [] or commentrpid == ['']:
        print('评论点赞失败\t\t\t原因：rpid获取失败')
    else:
        commentrpid = commentrpid[0]
        ...
        if pinglundianzanreq.text == '{"code":0,"message":"0","ttl":1,"data":{"suc_pic":"","suc_toast":""}}':
            print('评论点赞成功')
        else:
            print('评论点赞失败\t\t\t原因：request失败')
...
def xinxichuli(text):
    ...
    if typeif == '1':
        ...
        dongtaicontent2 = re.findall(r'"desc":{"uid":.*?"type":8.*?ctime.*?desc\\":\\".*?\\".*?dynamic\\":\\"(.*?)\\",',
                                     text)
        ...
        dongtaicontent = re.findall(r'"desc":{"uid":.*?"type":2048.*?\\"vest\\".*?"content\\": \\"(.*?)\\"',
                                    text)  # 动态内容
        dongtaineirong = ''.join(str(dongtaicontent[0]))
        pinglunrenshu = re.findall(r'comment":(.*?),"', text)[0]
        print(dongtaineirong)
    ...
    if int(pinglunrenshu) <= pinglunsuoxurenshu:
        ...
        n += 1
        print(
            'https://space.bilibili.com/' + str(fuid) + '\t' + 'https://t.bilibili.com/' + str(dynamic_id) + '\t' + str(
                dongtaineirong) + '\t')
        f.writelines(
            'https://space.bilibili.com/' + str(fuid) + '\t' + 'https://t.bilibili.com/' + str(dynamic_id) + '\t' + str(
                dongtaineirong) + '\t' + '\n')
        ...
        if msg != '':
            ...
            commentf.writelines(
                'https://t.bilibili.com/' + str(dynamic_id) + '\t' + str(msg) + '\t' + str(dongtaineirong) + '\n')
            time.sleep(random.choice(sleeptime))
            ...
            print('评论失败')
            choujianglianjie = 'https://t.bilibili.com/' + str(dynamic_id)
            pinglunshibai.apend(choujianglianjie)
            return 1
