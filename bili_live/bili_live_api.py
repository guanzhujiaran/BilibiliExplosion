...
class bili_live_api:
    ...
    def __init__(self):
        ...
        try:
            with open('K:\\python test\\哔哩哔哩直播\\roomd_id对应ruid.txt', 'r', encoding='utf-8') as r:
                if r:
                    for i in r:
                        self.room_id2ruid_dict.update({i.split('\t')[0]: i.split('\t')[1]})
        except:
            f = open('K:\\python test\\哔哩哔哩直播\\roomd_id对应ruid.txt', 'w', encoding='utf-8')
            f.close()
    ...
    def quit_write_in_file(self):
        for i, j in self.unknown_roomid.items():
            with open('K:\python test\\哔哩哔哩直播\\roomd_id对应ruid.txt', 'a+', encoding='utf-8') as r:
                r.writelines('{}\t{}\n'.format(i, j))
    ...
    def get_data_from_server(self):  # 1.12.36.165
        ...
        while 1:
            try:
                ...
                print(myp)
                time.sleep(1)
                req = self.s.get(url=url, headers=headers, timeout=5)
                break
    ...
    def popularity_red_pocket_join(self, uid, _id, roomid, ruid, cookie, ua, csrf_token):  # //参加直播间红包
        visitid = self.get_visit_id(uid)
        if uid is not None and _id is not None and roomid is not None and ruid is not None and cookie is not None and ua is not None and csrf_token is not None:
            self.roomEntryAction(cookie,visitid, ua, csrf_token, roomid)
            ...
            print(self.s.get(url=url, headers=headers).json())
    ...
    def sign_in(self, cookie, ua):  # 应援团签到
        ...
        if not req.get('code'):
            ...
            for group in group_list:
                ...
                data = json.dumps(data)
                sign_req = self.s.get(url=url, data=data, headers=headers)
