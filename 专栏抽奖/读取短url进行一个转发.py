...
class my_methods(Bilibili_methods.all_methods.methods):
    def comment_with_thumb(self, dy, msg, _type, rid, cookie, useragent, csrf, username):  # 发送评论
        ...
        if self._check_comment(username, dy, rid, _type):
            ...
            print('评论被吞')
            try:
                with open('../动态转抽_新版2.0/评论log/comment_log.csv', 'a+', encoding='utf-8') as c:
                    c.writelines(
                        '{0}\t{1}\t{2}\thttps://t.bilibili.com/{2}\t{3}\t{4}\n'.format(self.timeshift(time.time()),
                                                                                       username,
                                                                                       dy, repr(msg), comment_dict))
            except:
                with open('../动态转抽_新版2.0/评论log/comment_log.csv', 'w', encoding='utf-8') as c:
                    c.writelines(
                        '{0}\t{1}\t{2}\thttps://t.bilibili.com/{2}\t{3}\t{4}\n'.format(self.timeshift(time.time()),
                                                                                       username,
                                                                                       dy, repr(msg), comment_dict))
        ...
        if commentresult != "发送成功":
            ...
            self.pinglunshibai.append(cuowuid)
...
class mycls:
    ...
    def get_cv_info(self, uid, csrf, cookie, ua, msglist, username):
        ...
        prizedict_new = {}
        myindex = 1
        with open('短url.csv', 'r', encoding='utf-8') as q:
            for line in q:
                if not 'http' in line:
                    continue
                scode = re.findall('.*//.*?/(.*)', line.strip())[0]
                if len(scode) < 18:
                    pu = 'https://b23.tv/' + scode
                else:
                    pu = 'https://t.bilibili.com/' + scode
                # pu = line.strip()
                self.urlcount += 1
                prizedict_new.update({myindex: {myindex: pu}})
                myindex += 1
        print('此csv共有{c}条抽奖'.format(c=self.urlcount))
        for p_index in prizedict_new:
            for prizename in prizedict_new.get(p_index).keys():
                print('\t\t\t\t\t\t\t\t\t\t\t\t当前进度：{0}/{1}'.format(prizename, len(prizedict_new)))
                print(prizedict_new.get(p_index).get(prizename))
                l_url = prizedict_new.get(p_index).get(prizename)
                if 'https://b23.tv' in l_url:
                    shorturl = prizedict_new.get(p_index).get(prizename)
                    ...
                    print('长url:\t{}'.format(l_url))
                    dyid = l_url.split('/')[3].split('?')[0]
                if dyid:
                    if dyid not in self.canjiaguode:
                        time.sleep(random.choice(numpy.linspace(3, 5, 500, endpoint=True)))
                        self.start_lottery(dyid, uid, csrf, cookie, ua, msglist, username)
                        self.canjiaguode.append(dyid)
                    else:
                        print('参加过的动态')
                        self.changefail.append(l_url)
                else:
                    print('dyid分割出错')
                    self.changefail.append(l_url)

    def start_lottery(self, dynamicid, uid, csrf, cookie, useragent, msglist, username):
        ...
        if dyresponse:
            ...
            if str(type) == '2' or str(type) == '4' or str(type) == '8':
                if self.repostchance < random.random():
                ...
                self.repost(dynamicid, pinglunmsg, cookie, useragent, uid, csrf)
            time.sleep(1)
            myapi.comment(str(dynamicid), pinglunmsg, str(type), str(rid), cookie, useragent, csrf, username)
            time.sleep(random.choice(self.sleeptime))
    ...
    def huifuneirong(self, content, dynamic_id, rid, type):  # 回复内容的判断
        ...
        if pinglun_req != '{"code":-404,"message":"啥都木有","ttl":1}':
            if pinglun_count != 0:
                ...
                while True:
                    i += 1
                    pnlist = list(range(pinglun_count - 30, pinglun_count - 1))
                    if pnlist == []:
    ...
    def get_pinglunreq(self, dynamic_id, rid, pn, _type):
        ...
        try:
            ...
            print(self.timeshift(int(time.time())))
            # time.sleep(eval(input('输入等待时间')))
            # return self.get_pinglunreq(str(dynamic_id), str(rid), pn, str(type))
            return None
        return pinglunreq.text

    def get_dynamic_detail(self, dynamic_id, _cookie, _useragent):
        url = 'https://api.bilibili.com/x/polymer/web-dynamic/v1/detail?timezone_offset=-480&id=' + str(dynamic_id)
        headers = {
            "cookie": _cookie,
        ...
        try:
            dynamic_req = requests.request('GET', url=url, headers=headers)
        except Exception as e:
            # time.sleep(eval(input('输入等待时间')))
            time.sleep(3)
            print(e)
    ...
    def account_choose(self, accountname, *msglist):
        ...
        if accountname == 1:
            print(uid1)
            self.get_cv_info(uid=uid1, csrf=csrf1, cookie=cookie1, ua=ua1, msglist=msglist, username=username1)
        elif accountname == 2:
            print(uid2)
            self.get_cv_info(uid=uid2, csrf=csrf2, cookie=cookie2, ua=ua2, msglist=msglist, username=username2)
        elif accountname == 3:
            print(uid3)
            self.get_cv_info(uid=uid3, csrf=csrf3, cookie=cookie3, ua=ua3, msglist=msglist, username=username3)
        elif accountname == 4:
            print(uid4)
            self.get_cv_info(uid=uid4, csrf=csrf4, cookie=cookie4, ua=ua4, msglist=msglist, username=username4)
