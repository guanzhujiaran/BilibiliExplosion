...
class my_share:
    def __init__(self):
        ...
        self.m.ero_pic_up_list = [2350559,34144840]  # 色图up
        self.m.random_crate_dynamic_content = ['拿室友十年寿命许愿中大奖！']  # 自己创建动态的随机内容
        self.m.yunxingshijian = 0.5 * 3600  # 默认运行时间为半小时
        self.m.ero_pic_num = random.choice(range(6, 12))  # 色图的存量
        self.share_num = range(10, 14)  # 分享次数
        self.chance_list = [0.5, 1, 0.1, 1]  # 色图概率 热门视频概率  创建动态概率  换头像挂件概率
