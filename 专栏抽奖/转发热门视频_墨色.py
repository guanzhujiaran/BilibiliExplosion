...
class my_share:
    def __init__(self):
        self.m = Bi.module_pf()
        self.m.copy_comment_chance = 1  # 抄评论概率
        self.m.ero_pic_up_list = [1242384245, 37743601, 129060, 1930483018, 44714970]  # 色图up
        self.m.random_crate_dynamic_content = ['h', 'g', 'j', 'u', 'b']  # 自己创建动态的随机内容
        self.m.yunxingshijian = 0.5 * 3600  # 默认运行时间为半小时
        self.m.ero_pic_num = random.choice(range(6, 12))  # 色图的存量

        self.share_num = range(7, 16)  # 分享次数
        self.chance_list = [0, 1, 0.1, 0]  # 色图概率 热门视频概率  创建动态概率  换头像挂件概率
        ...
        for i in self.pf_type:
            if i == 1:
                ...
                for ii in range(crate_time):
                    print('创建动态')
                    self.m.module_random_crate_dynamic(random.choice(self.m.random_crate_dynamic_content), cookie, ua, csrf, uid)
