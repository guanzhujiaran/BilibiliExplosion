"""LangChain 模型服务，支持「外部 LLM API 列表 → 本地 Ollama」回退策略
...
按列表顺序依次尝试，全部失败后回退到本地 Ollama。未配置时直接使用本地 Ollama。
...
# ============ 全局 LLM 实例 ============
Qwen3_5_0_8_Q4_K_M_GGUF = "modelscope.cn/diodel/Qwen3.5-0.8B-Q4_K_M-GGUF:Q4_K_M"
# 本地 Ollama（兜底）
_local_llm = ChatOllama(
    model=Qwen3_5_0_8_Q4_K_M_GGUF,
...
def get_llm(force_local: bool = False, **kwargs: Any):
    ...
    从 llm_apis 列表中轮询选择主模型，其余作为 fallback，全部失败后回退到本地 Ollama。
    未配置外部 API 时直接使用本地 Ollama。
    ...
    if not kwargs:
        ...
        logger.info("无外部API配置，使用本地Ollama")
        return _local_llm

    local_bound = _local_llm.bind(**kwargs)
    if not has_free_api:
        logger.info("无外部API配置，使用本地Ollama（带参数绑定）")
        return local_bound
    ...
    logger.info(f"设置{len(fallbacks)}个fallback（含本地Ollama）")
    return primary.with_fallbacks(fallbacks)
