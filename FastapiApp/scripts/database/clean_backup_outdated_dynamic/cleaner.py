...
async def do_clean():
    first_dyn = await grpc_sql_helper.get_rid_bili_dyn_detail(is_asc=True)
    latest_dyn = await grpc_sql_helper.get_rid_bili_dyn_detail()
    if not latest_dyn:
        return
    ...
    between_ts = [first_dyn_ts, latest_dyn_ts - 90 * 24 * 60 * 60]
