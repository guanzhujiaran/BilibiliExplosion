...
def timeshift():
    ...
    return realtime
print(timeshift())
page=[2,3,4,5,6]#动态评论页面
yuzancishu=20#遇到点赞次数后停止
pinglunzhuanbiancishu=5
pinglunsuoxurenshu=30#动态下评论人数
uid=1178256718
global zhuanpingdongtaicishu
zhuanpingdongtaicishu = 0
sleeptime=numpy.linspace(1,3,100,endpoint=False)
wuxuzhuanfa=[]
csrf='5aef2590e38b30bf81da9faf5df34025'#填入自己的csrf
cookie="l=v; bsource=search_sougo; _uuid=F795431D-3032-5712-56EC-FB12379F142685832infoc; buvid3=0541832A-389A-44BD-BFBA-98D583C78163167641infoc; fingerprint=e92609c87c537643292c4653cd346aa2; buvid_fp=0541832A-389A-44BD-BFBA-98D583C78163167641infoc; buvid_fp_plain=0541832A-389A-44BD-BFBA-98D583C78163167641infoc; LIVE_BUVID=AUTO4816283075780167; SESSDATA=81555c44%2C1643861438%2C46da9%2A81; bili_jct=5aef2590e38b30bf81da9faf5df34025; DedeUserID=1178256718; DedeUserID__ckMd5=e2d2f6900a8e09dd; sid=5o6a55nu; _dfcaptcha=83ca444988aa521fc9ffd006bb501485; PVID=3; bp_t_offset_1178256718=556113306090741081"
typelist=268435455
useragent='Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.25 Safari/537.36 Core/1.70.3875.400 QQBrowser/10.8.4492.400'
...
changyongemo=['[吃瓜]','[doge]','[妙啊]','[嗑瓜子]']
f=open('非官方抽奖动态1.csv','w',encoding='utf-8')
a=open('点赞失败1.csv','w',encoding='utf-8')
p=open('oid变化得动态1.csv','w',encoding='utf-8')
q=open('原视频下方评论1.csv','w',encoding='utf-8')
r=open('无需转发1.csv','w',encoding='utf-8')
commentf=open('评论相关1.csv','w',encoding='utf-8')
xiaowanyi=open('小礼物1.csv','w',encoding='utf-8')
f.writelines('发布者uid'+'\t'+'动态链接'+'\t'+'动态内容''\n')
def huifuneirong(content,detail,type):#回复内容的判断
    topobj_1 = re.match('.*@你想祝福的人.*', content)
    topobj0 = re.match('.*@1位胖友.*', content)
    topobj1 = re.match('.*圈1位你的伙伴.*', content)
    topobj2 = re.match('.*带tag#.*#.*', content)
    topobj3 = re.match('.*带话题#.*#.*', content)
    topobj4 = re.match('.*带上tag#.*#.*', content)
    topobj5 = re.match('.*带#.*#.*', content)
    if topobj_1 != None:
        msg = '@_大锦鲤_  '
        return msg
    if topobj0 != None:
    ...
    if topobj3 != None:
        ...
        return msg
    if topobj4 != None:
        msg = re.findall(r'.*?带上tag#(.*?)#.*?', content)
        msg = '#' + str(msg[0]) + '#'
        return msg
    if topobj4 != None:
        msg = re.findall(r'.*?带#(.*?)#.*?', content)
        if msg!='' and msg!=[] and msg!=None and msg!=[''] and msg!=' ':
            msg = '#' + str(msg[0]) + '#'
            return msg
    MAX = page.__len__() - 1
    ctype = 17
...
def choujiangxinxipanduan(tcontent):  # 动态内容过滤条件
    ...
    matchobj2 = re.match('.*转.{0,3}评.*', tcontent, re.DOTALL)
    matchobj3 = re.match('.*本条.*送.*', tcontent, re.DOTALL)
    matchobj5 = re.match('.*抽.{3,7}送.*', tcontent, re.DOTALL)
    matchobj10 = re.match('.*钓鱼.*', tcontent, re.DOTALL)
    matchobj11 = re.match('.*领.*？奖.*', tcontent, re.DOTALL)
    ...
    matchobj29 = re.match('.*最大贝者场游戏规则公告大家以后要来碰碰运气一定要记住按照最后在碗里可见的卷数为准垫底的卷无效喔快来许愿碰碰运气吧.*', tcontent, re.DOTALL)
    matchobj30 = re.match('.*恭喜.*获得.*', tcontent, re.DOTALL)
    matchobj33 = re.match('.*快快点击传送门一起抽大奖！！.*', tcontent, re.DOTALL)
    matchobj34 = re.match('.*转发抽奖结果.*', tcontent, re.DOTALL)
    matchobj36 = re.match('.*开奖啦！.*', tcontent, re.DOTALL)
    matchobj37 = re.match('.*奖品转送举报人.*', tcontent, re.DOTALL)
    matchobj38 = re.match('.*来开奖！.*', tcontent, re.DOTALL)
    matchobj39=re.match('.*200元优惠券.*',tcontent, re.DOTALL)
    matchobj40=re.match('.*恭喜.*中奖.*',tcontent, re.DOTALL)
    matchobj42=re.match('.*福利公示.*',tcontent, re.DOTALL)
...
def thumb(dynamic_ID):#为动态点赞标记
    ...
    'path': '/ dynamic_like / v1 / dynamic_like / thumb',
     'scheme': 'https',
    'cookie': "SESSDATA=81555c44%2C1643861438%2C46da9%2A81;bili_jct=5aef2590e38b30bf81da9faf5df34025",
    'user-agent':useragent
    }
    thumbdata={
        'uid': uid,
...
def xinxichuli(text):
    ...
    dongtaineirong = ''
    try:
        dynamic_id = re.findall(r'"desc":{"uid":.+?dynamic_id":(.*?),"timestamp', text)[0]
        ...
        uname = ''.join(re.findall(r'"uname":"(.*?)"', text)[0])
    except Exception as e:
        print(text)
        if text=='{"code":-412,"message":"请求被拦截","ttl":1,"data":null}':
    ...
    if typeif == '1':
        ...
        print('番剧动态 https://t.bilibili.com/' + str(dynamic_id))
        n += 1
        print('第' + str(n) + '次获取动态' + '\n')
        time.sleep(random.choice(sleeptime))
        return 1
    ...
    if int(pinglunrenshu) <= pinglunsuoxurenshu:
        ...
        obj32 = re.match('.*视频.*一键三联.*', str(dongtaineirong))
        obj33 = re.match('.*弹幕.*评论.*选.*', str(dongtaineirong))
        obj34 = re.match('.*文章内.*评论区.*', str(dongtaineirong))
        obj35 = re.match('.*抽奖传送门.*', str(dongtaineirong))
        obj36 = re.match('.*置顶动态.*', str(dongtaineirong))
        obj37 = re.match('.*原抽奖动态.*', str(dongtaineirong))
        if (obj10 != None or obj9 != None or obj8 != None or obj7 != None or obj6 != None or obj5 != None or obj1 != None or
            obj2 != None or obj3 != None or obj4 != None or matchobj001 != None or matchobj002 != None or matchobj003 != None or
            matchobj004 != None or obj11 != None or obj12 != None or obj13 != None or obj15 != None or obj16 != None
            ...
            print('第' + str(n) + '次获取动态')
            lianjie = 'https://t.bilibili.com/' + str(dynamic_id)
            detaillist = [lianjie, str(dongtaineirong)]
        ...
        xiaoliwuobj83 = re.match('.*七工匠周边.*', str(dongtaineirong))
        xiaoliwuobj84 = re.match('.*派乐礼盒.*', str(dongtaineirong))
        xiaoliwuobj85 = re.match('.*权益体验卡/九阳修缘盒.*', str(dongtaineirong))
        xiaoliwuobj86 = re.match('.*权益体验卡or九阳修缘丹.*', str(dongtaineirong))
        xiaoliwuobj87 = re.match('.*权益体验卡.*', str(dongtaineirong))
        if (xiaoliwuobj != None or xiaoliwuobj1 != None or xiaoliwuobj3 != None
            ...
            or xiaoliwuobj29 != None or xiaoliwuobj30 != None or xiaoliwuobj31 != None or xiaoliwuobj33 != None
            ...
            or xiaoliwuobj49 != None or xiaoliwuobj50 != None or xiaoliwuobj51 != None or xiaoliwuobj52 != None or xiaoliwuobj53 != None
            or xiaoliwuobj54 != None or xiaoliwuobj55 != None or xiaoliwuobj56 != None or xiaoliwuobj57 != None or xiaoliwuobj58 != None
            or xiaoliwuobj60 != None or xiaoliwuobj61 != None or xiaoliwuobj62 != None or xiaoliwuobj63 != None
            or xiaoliwuobj63 != None or xiaoliwuobj64 != None or xiaoliwuobj65 != None or xiaoliwuobj66 != None or xiaoliwuobj67 != None
            or xiaoliwuobj68 != None or xiaoliwuobj69 != None or xiaoliwuobj70 != None or xiaoliwuobj71 != None or xiaoliwuobj72 != None
            or xiaoliwuobj73 != None or xiaoliwuobj74 != None or xiaoliwuobj75 != None or xiaoliwuobj76 != None or xiaoliwuobj77 != None
            or xiaoliwuobj78 != None or xiaoliwuobj79 != None or xiaoliwuobj80 != None or xiaoliwuobj81 != None or xiaoliwuobj82 != None
            or xiaoliwuobj83 != None or xiaoliwuobj84 != None or xiaoliwuobj85 != None or xiaoliwuobj86 != None or xiaoliwuobj87 != None
        ):
            n += 1
            print('第' + str(n) + '次获取动态')
            lianjie = 'https://t.bilibili.com/' + str(dynamic_id)
        ...
        zhuanfapanduan4 = re.match('.*互动抽奖.*', str(dongtaineirong))
        if (zhuanfapanduan1!=None or zhuanfapanduan2!=None or zhuanfapanduan3!=None
            or zhuanfapanduan4 != None
        ):
...
class start:
    ...
    while True:
        ...
        try:offset = re.findall(r'"next_offset":(.*?),"', req.text)[0]
        except:print(req.text)
        print('\n\n\n' + 'next_offset:' + offset)
        dynamic_id = re.findall(r'"desc":{"uid":.+?dynamic_id":(.*?),"timestamp', req.text)
        print('此页共有' + str(dynamic_id.__len__()) + '条动态')
        for dynamic in dynamic_id:
            dydetail = get_dynamic_detail(dynamic)
            xinxichulires=xinxichuli(dydetail)
            while xinxichulires==412:
                dydetail = get_dynamic_detail(dynamic)
                xinxichulires = xinxichuli(dydetail)
            if n%500==0:
                print('到达500条休息30秒\n\n\n\n\n')
                time.sleep(30)
    import 究极最终改不动的.转发热门视频_墨色 as rep
    rep._main()
f.close()
