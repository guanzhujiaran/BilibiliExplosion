...
class generate_cv:
    ...
    def zhuanlan_date_sort(self, zhuanlan_data_order_by_date: list, limit_date_switch: bool = False,
        ...
        for lottery_data in zhuanlan_data_order_by_date:
            lottery_end_date = datetime.datetime.strptime(
                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(lottery_data['etime']))), '%Y-%m-%d %H:%M:%S')
            only_date = lottery_end_date.date()
            if limit_date_switch:
                if (only_date - next_day.date()).days > limit_date:  # 大于指定天数的抽奖不放进去
                    continue
            if int(lottery_data['etime']) < time.time():
                # 如果过期了进入下一条
                continue
            ...
            if oneDayList.get(str(lottery_end_date.date())):  # 如果存在当前抽奖日期，则直接append上去
                ...
                for __ in oneDayList.get(str(lottery_end_date.date())):
                    if __.get('ids') == lottery_data.get('ids'):
                        chongfu_Flag = True
                if not chongfu_Flag:
    ...
    def sort_reserve_lottery(self, zhuanlan_data: [dict], up_dated_flag=False):
        '''
        预约数据分类
        [ 'code', 'message', 'ttl', 'sid', 'name', 'total', 'stime',
       'etime', 'isFollow', 'state', 'oid', 'type', 'upmid',
       'reserveRecordCtime', 'livePlanStartTime', 'upActVisible',
       'lotteryType', 'text', 'jumpUrl', 'dynamicId', 'reserveTotalShowLimit',
       'desc', 'start_show_time', 'hide', 'subType', 'productIdPrice',
        'ids', 'reserve_products']
        :param up_dated_flag: 是否更新log
        :param zhuanlan_data:
        :return:
        '''
        invalid_reserve_list = []
        invalid_reserve_file_path = 'log/失效的预约抽奖.csv'
        drawn_reserve_list = []
        drawn_reserve_file_path = 'log/开了的预约抽奖.csv'
        undrawn_reserve_list = []

        for reserve_data in zhuanlan_data:
            if str(reserve_data['state']) == '-100' or str(reserve_data['state']) == '-300':
                invalid_reserve_list.append(reserve_data)
            elif str(reserve_data['state']) == '-110' or str(reserve_data['state']) == '150':
                drawn_reserve_list.append(reserve_data)
            else:
                if pandas.notnull(reserve_data['etime']):
                    if (int(reserve_data['etime'])>int(time.time())):
    ...
    def reserve_lottery(self):
        ...
        :return:
        '''
        zhuanlan_data = pandas.read_csv('result/全部抽奖预约.csv', dtype=str, index_col=0,sep='滹').to_dict('records')
        ...
        zhuanlan_data1 = pandas.read_csv('result/更新的抽奖预约.csv', dtype=str, index_col=0,sep='滹').to_dict('records')
